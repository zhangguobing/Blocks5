...
final class QRCodeEncoder2 {
    ...
    QRCodeEncoder2(QREncode.Builder build, Context context) {
    ...
    Bitmap encodeAsBitmapWithoutWhite(int dimension) throws WriterException {
        if (encodeBuild.getColor() == 0)
            encodeBuild.setColor(BLACK);
        String contentsToEncode = encodeBuild.getEncodeContents();
        if (contentsToEncode == null) {
        ...
        BitMatrix matrix = new QRCodeWriter().encode(contentsToEncode, BarcodeFormat.QR_CODE, dimension, dimension);
